...
from model.resnet_noPADAA import wide_resnet50_2
...
from dataset.FPAG_dataset import MVTecDataset_test, FPAG, get_data_transforms,MVTecDataset_train_original
...
def get_args():
    ...
    parser.add_argument('--classes', nargs="+",
                        default=['carpet','grid','leather','tile', 'wood', 'bottle', 'cable', 'capsule', 'hazelnut', 'metal_nut',
...
def train(_class_, pars):
    ...
    train_data = MVTecDataset_train_original(root=train_path, transform=data_transform)
    ...
    for epoch in tqdm(range(1, num_epoch + 1)):
        ...
        for i, (img, img_noise, _) in enumerate(train_dataloader):
            ...
            # L_proj = proj_loss(feature_space_noise, feature_space)
            L_proj = proj_loss(inputs, inputs_noise)
            ...
            loss.backward()
            # loss = (L_distill + pars.weight_proj * L_proj) / accumulation_steps
            # loss.backward(retain_graph=(i + 1) % accumulation_steps != 0)

            if (i + 1) % accumulation_steps == 0:

                max_grad_norm = 5.0
                # torch.nn.utils.clip_grad_norm_(proj_layer.parameters(), max_grad_norm)
                torch.nn.utils.clip_grad_norm_(decoder.parameters(), max_grad_norm)
                torch.nn.utils.clip_grad_norm_(bn.parameters(), max_grad_norm)
