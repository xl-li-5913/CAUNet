...
class Simplex_CLASS:
    ...
    def __init__(self):
        self.newSeed() # 初始化时生成一个新的随机种子

    def newSeed(self, seed=None):
        if not seed:
            seed = np.random.randint(-100000, 100000) # 在 [-100000, 100000) 范围内生成随机整数
        # 初始化噪声参数
        self._perm, self._perm_grad_index3 = _init(seed) # 调用 _init 函数生成排列表和梯度索引表
...
def _init(seed=DEFAULT_SEED):
    ...
    # 初始化排列表和梯度索引表，用零填充以便后续循环处理
    perm = np.zeros(256, dtype=np.int64)
    perm_grad_index3 = np.zeros(256, dtype=np.int64)
    source = np.arange(256) # 创建一个包含 0 到 255 的数组，用于生成随机排列
    # Generates a proper permutation (i.e. doesn't merely perform N
    # successive pair swaps on a base array)
    seed = overflow(seed * 6364136223846793005 + 1442695040888963407) # 第一次更新种子
    seed = overflow(seed * 6364136223846793005 + 1442695040888963407) # 第二次更新种子
    seed = overflow(seed * 6364136223846793005 + 1442695040888963407) # 第三次更新种子
    for i in range(255, -1, -1): # 从后向前遍历，生成随机排列
        seed = overflow(seed * 6364136223846793005 + 1442695040888963407)
        r = int((seed + 31) % (i + 1)) # 计算随机索引 r，范围为 [0, i]
        if r < 0: # 确保 r 不为负数
            r += i + 1
        perm[i] = source[r]
        perm_grad_index3[i] = int((perm[i] % (len(GRADIENTS3) / 3)) * 3)
        source[r] = source[i] # 将 source[i] 的值赋给 source[r]，避免重复选择
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="给图像添加 OpenSimplex 噪声")
    parser.add_argument('input',  help="./data/hazelnut/train/good/000.png")
    parser.add_argument('output', help="./result")
    parser.add_argument('--octaves',   type=int,   default=4,    help="噪声分形层数（octaves）")
    parser.add_argument('--persistence', type=float, default=0.5,  help="噪声持久度（persistence）")
    parser.add_argument('--frequency',   type=float, default=32.0, help="初始频率（frequency）")
    parser.add_argument('--strength',    type=float, default=0.2,  help="噪声强度（0~1）")
    ...
    noise_gen = Simplex_CLASS()
    noise = noise_gen.rand_2d_octaves(
        shape=(h, w),
    ...
    out = arr + args.strength * noise_rgb
