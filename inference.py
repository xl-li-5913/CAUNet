...
from utils.SAKT import SAKTLayer
from dataset.FPAG_dataset import MVTecDataset_test, get_data_transforms
...
def inference(_class_, pars):
    ...
    sakt_layer.load_state_dict(ckp['proj'])
    padaa.load_state_dict(ckp['bn'])
    decoder.load_state_dict(ckp['decoder'])

    auroc_px, auroc_sp, aupro_px = evaluation_multi_proj(encoder, sakt_layer, padaa, decoder, test_dataloader, device,save_dir=f"{pars.checkpoint_folder}/{c}/vis")
